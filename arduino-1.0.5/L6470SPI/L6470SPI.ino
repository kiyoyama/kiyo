...
void messageCb(const geometry_msgs::Pose& pose_msgs)
{
   ...
   if(flag == 0){
      nh.loginfo("move");
      /*publishされた値を時間に換算し、伸縮する。
      
      run(spd)関数は(0.015*spd[step/s])  run(0,spd)　上がる　run(1,spd)　下がる
      1step = 0.9deg 1リードmm
      150step/sは1.125mmうごくので、 450step/s = 3.375 [mm]
      */
      float grow = z /3.375;
      float time = grow * 1000;
      L6470_run(0,30000);//逆転方向に150step/s * 3 = 450step/sの速さで回転
      L6470_busydelay(time);//前の処理をtimeだけ行う
      L6470_hardhiz();
...
L6470_setparam_kvalhold(0x40); //[R, WR]停止時励磁電圧default 0x29 (8bit) (Vs[V]*val/256)
L6470_setparam_kvalrun(0x40); //[R, WR]定速回転時励磁電圧default 0x29 (8bit) (Vs[V]*val/256)
L6470_setparam_kvalacc(0x40); //[R, WR]加速時励磁電圧default 0x29 (8bit) (Vs[V]*val/256)
L6470_setparam_kvaldec(0x40); //[R, WR]減速時励磁電圧default 0x29 (8bit) (Vs[V]*val/256)
